...
def render_tab(segment: Segment, output_path="guitar_tab_v3.png"):
    ...
    img_width = int(total_units * BEAT_WIDTH) + MARGIN_LEFT + 150
    ...
    acc_dur = 0 # Accumulated duration in current measure
    ...
    # We need to peek ahead to see if we can form a pair for the "comb"
    for idx, note in enumerate(notes):
        # 1. Measure Boundary Reset
        ...
        if note.chord and note.style != StrumStyle.NO_HIT:
            ...
            # 3. Rhythmic Stems & Beams
            stem_x = current_x + 4
            bottom_y = STEM_Y_START + STEM_HEIGHT
            
            # Draw the vertical stem for both duration 1 and 2
            draw.line([(stem_x, STEM_Y_START), (stem_x, bottom_y)], fill="black", width=2)

            if note.duration == 1:
                # Check for "Comb" (Pairing)
                # Must be at the start of a beat (0, 2, 4, 6) AND next note must be duration 1
                can_beam = False
                ...
                # Check if we are the SECOND half of a beam started by the previous note
