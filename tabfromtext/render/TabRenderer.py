...
def draw_arc(draw_obj, utils: LayoutUtils, x_start, x_end, y_top):
    cfg     = utils.cfg
    top_off = utils.px(cfg.arc_top_offset_pt)
    bot_off = utils.px(cfg.arc_bot_offset_pt)
    arc_box = [x_start, y_top - top_off, x_end, y_top - bot_off]
    draw_obj.arc(arc_box, start=180, end=0, fill="black",
                 width=utils.lw(cfg.line_width_thin_pt))


def draw_stem(draw_obj, utils: LayoutUtils, stem_x, stem_y_start, duration,
              is_rest: bool = False):
    cfg = utils.cfg
    if is_rest:
        full_h = utils.px(cfg.stem_h_pt)
        rest_h = utils.px(cfg.rest_stem_pt)
        draw_obj.line([(stem_x, stem_y_start + full_h - rest_h),
                       (stem_x, stem_y_start + full_h)],
                      fill="black", width=utils.lw(cfg.line_width_normal_pt))
        return
    if duration >= TICKS_FULL_NOTE:
        draw_obj.line([(stem_x, stem_y_start),
                       (stem_x, stem_y_start + utils.px(cfg.stem_h_pt * 3.0))],
                      fill="black", width=utils.lw(cfg.line_width_normal_pt))
    elif duration >= TICKS_HALF_NOTE:
        top_h = utils.px(cfg.stem_h_pt * 0.5)
        gap   = utils.px(cfg.stem_h_pt * 1.0)
        bot_h = utils.px(cfg.stem_h_pt * 1.5)
        draw_obj.line([(stem_x, stem_y_start),
        ...
        draw_obj.line([(stem_x, stem_y_start),
                       (stem_x, stem_y_start + utils.px(cfg.stem_h_pt))],
                      fill="black", width=utils.lw(cfg.line_width_normal_pt))


def draw_staff_elements(draw_obj, utils: LayoutUtils, fret_font, small_font,
    ...
    font_h       = utils.px(cfg.string_name_font_size_pt)
    mnum_y_off   = utils.px(cfg.measure_num_y_offset_pt)

    for i, name in enumerate(string_names):
        y = y_top + i * utils.line_sp_px
        draw_obj.text((utils.margin_left_px - utils.px(cfg.string_name_x_pt) - font_h,
                       y - font_h // 2),
                      name, fill="black", font=string_name_font)
        draw_obj.line([(utils.margin_left_px, y),
                       (utils.margin_left_px + utils.content_w_px, y)],
                      fill=(200, 200, 200), width=utils.lw(cfg.line_width_thin_pt))

    draw_obj.line([(utils.margin_left_px, y_top),
                   (utils.margin_left_px, y_top + 5 * utils.line_sp_px)],
                  fill="black", width=utils.lw(cfg.line_width_normal_pt))
    draw_obj.text((utils.margin_left_px, y_top + mnum_y_off),
                  str(start_measure_num), fill="gray", font=small_font)


def draw_dashed_segment(draw_obj, utils: LayoutUtils, x_start, x_end, y):
    dash_gap = utils.px(utils.cfg.dash_gap_pt)
    curr = x_start
    while curr < x_end:
        draw_obj.line([(curr, y), (min(curr + dash_gap, x_end), y)],
                      fill="black", width=utils.lw(utils.cfg.line_width_thin_pt))
        curr += dash_gap * 2
...
def render_tab(segments: list[Segment], instrument_name: str,
    ...
    for seg_idx, segment in enumerate(segments):
        ...
        img_height_px = (utils.px(cfg.title_padding_pt)
                         + utils.title_h_px
        ...
        # --- Title ---
        top_padding_px = utils.px(cfg.title_padding_pt)
        ...
        for idx, note in enumerate(segment_notes):
            ...
            if note.duration is not None:
                ...
                final_x = next_x
                if is_new_line:
                    draw_staff_elements(draw, utils, fret_font, small_font,
                ...
                if (utils.tick_to_unit_in_measure(acc_dur_segment) == 0
                    ...
                    bar_x = utils.barline_x(acc_dur_segment)
                    draw.line([(bar_x, strings_y),
                               (bar_x, strings_y + 5 * utils.line_sp_px)],
                              fill="black", width=utils.lw(cfg.line_width_normal_pt))
                    draw.text((bar_x, strings_y + utils.px(cfg.measure_num_y_offset_pt)),
            ...
            if note.duration is not None:
                ...
                while remaining_dur > 0:
                    ...
                    chunk_stem_x       = chunk_x + utils.px(cfg.stem_x_offset_pt)
                    ...
                    if utils.is_new_measure(chunk_acc) and chunk_acc != acc_dur_segment:
                        bar_x = utils.barline_x(chunk_acc)
                        draw.line([(bar_x, chunk_strings_y),
                                   (bar_x, chunk_strings_y + 5 * utils.line_sp_px)],
                                  fill="black", width=utils.lw(cfg.line_width_normal_pt))
                        draw.text((bar_x, chunk_strings_y + utils.px(cfg.measure_num_y_offset_pt)),
                                  str(global_measure_counter), fill="gray", font=small_font)
                    ...
                    if note.chord and note.style != StrumStyle.NO_HIT:
                        ...
                        for i, fret in enumerate(strings):
                            if fret is not None and fret != -1:
                                y     = chunk_strings_y + i * utils.line_sp_px
                                ...
                                if note.style == StrumStyle.SLIDE:
                                    nudge = utils.px(cfg.slide_nudge_pt)
                                    draw.line([(chunk_x + text_w + nudge, y),
                                               (chunk_next_x - nudge, y)],
                                              fill="black", width=utils.lw(cfg.line_width_thin_pt))
                                    draw_arc(draw, utils,
                                             chunk_x + nudge, chunk_next_x - nudge,
                                             chunk_strings_y + i * utils.line_sp_px)

                        if note.style == StrumStyle.PALM_MUTED:
                            pm_y = chunk_strings_y + utils.px(cfg.pm_y_offset_pt)
                            is_first_pm_on_line = (last_style != StrumStyle.PALM_MUTED
                                                   or chunk_is_new_line)
                            next_real_note = next(
                            ...
                            if is_first_pm_on_line:
                                draw.text((chunk_x, pm_y - utils.px(cfg.pm_label_y_pt)),
                                          "P.M.", fill="black", font=annotation_font)
                                last_pm_x = chunk_x + utils.px(cfg.pm_label_w_pt)
                            else:
                                draw_dashed_segment(draw, utils, last_pm_x, chunk_x, pm_y)
                                last_pm_x = chunk_x
                            if is_last_pm and not is_first_pm_on_line:
                                tick_h = utils.px(cfg.tick_h_pt)
                                draw.line([(chunk_x, pm_y - tick_h // 2),
                                           (chunk_x, pm_y + tick_h // 2)],
                                          fill="black", width=utils.lw(cfg.line_width_thin_pt))
                            last_pm_y = pm_y

                    is_rest = (note.chord is None or note.style == StrumStyle.NO_HIT)
                    draw_stem(draw, utils, chunk_stem_x, chunk_stem_y_start,
                              chunk_dur, is_rest=is_rest)

                    if chunk_acc == acc_dur_segment and is_dotted(note.duration):
                        dot_offset = utils.px(cfg.dot_offset_pt)
                        dot_r      = utils.px(cfg.dot_r_pt)
                        dot_x = chunk_stem_x + dot_offset
                        dot_y = chunk_stem_y_start + dot_offset
                        draw.ellipse([dot_x - dot_r, dot_y - dot_r,
                                      dot_x + dot_r, dot_y + dot_r], fill="black")

                    if prev_stem_x is not None:
                        draw_arc(draw, utils, prev_stem_x, chunk_stem_x,
                                 min(prev_stem_y_start, chunk_stem_y_start))

                    if chunk_dur in (TICKS_EIGHTH, TICKS_DOTTED_EIGHTH):
                        chunk_bottom_y = chunk_stem_y_start + utils.px(cfg.stem_h_pt)
                        next_real_idx  = next(
                        ...
                        if can_beam_fwd:
                            draw.line([(chunk_stem_x, chunk_bottom_y),
                                       (chunk_stem_x + utils.beat_w_px * chunk_dur,
                                        chunk_bottom_y)],
                                      fill="black", width=utils.lw(cfg.line_width_thick_pt))
                        elif not is_beamed_back:
                            draw.line([(chunk_stem_x, chunk_bottom_y),
                                       (chunk_stem_x + utils.px(cfg.beam_stub_pt),
                                        chunk_bottom_y)],
                                      fill="black", width=utils.lw(cfg.line_width_normal_pt))
                    ...
                    if utils.is_new_system(chunk_acc):
                        ...
                        draw.line([(utils.margin_left_px + utils.content_w_px, eol_strings_y),
                                   (utils.margin_left_px + utils.content_w_px,
                                    eol_strings_y + 5 * utils.line_sp_px)],
                                  fill="black", width=utils.lw(cfg.line_width_normal_pt))
        ...
        if segment.lyrics is not None:
            ...
            lyrics_y_off_px = utils.px(cfg.lyrics_y_offset_pt)
            ...
            for abs_tick, syl_text in syllable_events:
                ...
                draw.text((syl_x_left - text_w // 2, syl_y),
...
def render_title_page(song: Song, cfg: LayoutConfig = None,
    ...
    title_line_h  = int(utils.px(cfg.title_font_size_pt)  * 1.4)
    lyrics_line_h = int(utils.px(cfg.lyrics_font_size_pt) * 1.4)
    section_gap   = lyrics_line_h
    top_pad_px    = utils.px(cfg.page_top_margin_pt * 0.5)
